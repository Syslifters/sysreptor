...
from reportcreator_api.rendering import tasks
...
@log_timing
def render_pdf_task(project_type: ProjectType, report_template: str, report_styles: str, data: dict, password: Optional[str] = None, project: Optional[PentestProject] = None):
    res = tasks.render_pdf_task.delay(
            ...
            password=password,
            resources={
                '/assets/name/' + a.name: b64encode(a.file.read()).decode() for a in project_type.assets.all()
            } | {
                '/images/name/' + i.name: b64encode(i.file.read()).decode() for i in (project.images.all() if project else [])
            }
    ).get()
...
def render_pdf(project: PentestProject, project_type: Optional[ProjectType] = None, report_template: Optional[str] = None, report_styles: Optional[str] = None, password: Optional[str] = None) -> bytes:
    if not project_type:
    ...
    data = {
        ...
        'pentesters': [format_template_field_user(u) for u in project.members.all()],
    }
    data = format_template_data(data=data, project_type=project_type, imported_members=project.imported_members)
    return render_pdf_task(
        project=project,
