...
from reportcreator_api.conf import settings
...
class PentestUserQuerySet(models.QuerySet):
    ...
    def only_permitted(self, user):
        ...
        if user.is_anonymous:
            return self.none()
        if user.is_guest and not settings.GUEST_USERS_SEE_ALL_USERS and not (user.is_superuser or user.is_user_manager):
