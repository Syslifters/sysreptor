...
from sysreptor.pentests.fielddefinition.predefined_fields import FINDING_FIELDS_CORE, REPORT_FIELDS_CORE
...
from sysreptor.pentests.models.files import UploadedAsset, UploadedImage
from sysreptor.pentests.models.notes import ProjectNotebookPage
...
from sysreptor.pentests.tasks import cleanup_history
...
from sysreptor.utils.fielddefinition.types import FieldDefinition, StringField, serialize_field_definition
...
class TestProjectHistory:
    ...
    def test_history_api(self):
        ...
        for h in history:
            ...
            assert {s['id']: omit_keys(s, ['updated']) for s in res_p.data['sections']} == {s['id']: omit_keys(s, ['updated']) for s in h['project']['sections']}
            ...
            for fh in h['findings']:
                ...
                assert omit_keys(res_f.data, ['updated']) == omit_keys(fh, ['updated'])
            for sh in h['sections']:
                res_s = self.client.get(reverse('pentestprojecthistory-section', kwargs=url_kwargs | {'id': sh['id']}))
                assert omit_keys(res_s.data, ['updated']) == omit_keys(sh, ['updated'])
            for nh in h['notes']:
    ...
    def test_history_api_show_deleted(self):
        ...
        assert res_deleted.status_code == 200
        assert omit_keys(res_deleted.data, ['updated']) == omit_keys(res_before_delete, ['updated'])
