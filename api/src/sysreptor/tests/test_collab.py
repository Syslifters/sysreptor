...
class TestProjectNotesDbSync:
    ...
    async def test_shareinfo_sync(self):
        res = await sync_to_async(self.api_client1.post)(
            path=reverse('projectnoteshareinfo-list', kwargs={'project_pk': self.project.id, 'note_id': self.note.note_id}),
...
@pytest.mark.django_db(transaction=True)
@pytest.mark.asyncio()
class TestSharedUserNotesDbSync:
    @pytest_asyncio.fixture(autouse=True)
    async def setUp(self):
        def setup_db():
            self.user = create_user(notes_kwargs=[])
            note_kwargs = {'checked': None, 'icon_emoji': None, 'title': 'ABC', 'text': 'ABC'}
            self.note_shared = create_usernotebookpage(user=self.user, **note_kwargs)
            self.childnote_shared = create_usernotebookpage(user=self.user, parent=self.note_shared, **note_kwargs)
            self.note_not_shared = create_usernotebookpage(user=self.user, **note_kwargs)
            self.childnote_not_shared = create_usernotebookpage(user=self.user, parent=self.note_not_shared, **note_kwargs)
        ...
        async with ws_connect(path='/api/ws/pentestusers/self/notes/', user=self.user) as self.client_user, \
                   ws_connect(path=f'/api/public/ws/shareinfos/{self.share_info.id}/notes/', user=None, other_clients=[self.client_user]) as self.client_public:
            yield
...
class TestConsumerPermissions:
    ...
    async def test_shared_project_notes_permissions(self, share_kwargs, expected_read, expected_write):
        def setup_db():
    ...
    @pytest.mark.parametrize(('share_kwargs', 'expected_read', 'expected_write'), [
        ({'is_revoked': True}, False, False),
        ({'user': {'is_active': False}}, False, False),
        ({'expire_date': (timezone.now() - timedelta(days=1)).date()}, False, False),
        ({'password': 'password'}, False, False),
        ({'permissions_write': False}, True, False),
        ({'permissions_write': True}, True, True),
    ])
    async def test_shared_user_notes_permissions(self, share_kwargs, expected_read, expected_write):
        def setup_db():
            user = create_user(notes_kwargs=[{'type': NoteType.TEXT}], **share_kwargs.pop('user', {}))
            note = user.notes.filter(type=NoteType.TEXT).first()
            create_usernotebookpage(user=user, parent=note, order=1, type=NoteType.EXCALIDRAW)
            return create_shareinfo(usernote=note, **share_kwargs)
        share_info = await sync_to_async(setup_db)()
        note_excalidraw = await share_info.usernote.user.notes.filter(type=NoteType.EXCALIDRAW).aget()
        ...
        res = await sync_to_async(client.get)(reverse('sharednote-list', kwargs={'shareinfo_pk': share_info.id}))
        assert res.status_code in ([200] if expected_read else [403, 404])
        res = await sync_to_async(client.patch)(reverse('sharednote-detail', kwargs={'shareinfo_pk': share_info.id, 'id': share_info.usernote.note_id}), data={})
        assert res.status_code in ([200] if expected_write else [403, 404])
